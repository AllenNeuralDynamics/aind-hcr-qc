...
import seaborn as sns
import numpy as np
from matplotlib_venn import venn2, venn3  # You might need to install this with: pip install matplotlib-venn
...
def plot_pairwise_intensities_multi(df, cell_ids, plot_type='individual', scale='log'):
    """
    ...
    plot_type: 'individual' for separate plots per cell, 
               'combined' for all cells in one plot,
               'grid' for a grid comparison
    ...
    # Create color mapping for unmixed channels
    channel_colors = {
        '488': 'green',
        '561': 'orange',
        '638': 'red'
    ...
    channel_pairs = [
        ('chan_488_intensity', 'chan_561_intensity', '488 vs 561'),
        ('chan_488_intensity', 'chan_638_intensity', '488 vs 638'),
        ('chan_561_intensity', 'chan_638_intensity', '561 vs 638')
    ]
    
    def transform_intensity(intensity, scale):
        """Transform intensity based on selected scale"""
        return np.log1p(intensity) if scale == 'log' else intensity
    
    def plot_pairwise_for_subset(cell_data, ax, x_col, y_col, scale):
        # Extract the channel numbers from column names
        x_chan = x_col.split('_')[1]
        y_chan = y_col.split('_')[1]
        
        # Find unique channels that are specifically in the current pair
        unique_channels = cell_data[cell_data['unmixed_chan'].isin([x_chan, y_chan])]['unmixed_chan'].unique()
        
        # Scatter storage for both matching and non-matching spots
        matching_scatter_data = {}
        non_matching_scatter_data = {}
        
        for chan in unique_channels:
            chan_data = cell_data[cell_data['unmixed_chan'] == chan]
            
            # Separate matching and non-matching spots
            matching_spots = chan_data[chan_data['chan'] == chan]
            non_matching_spots = chan_data.loc[chan_data['chan']!=chan]#chan_data.loc[chan_data['unmixed_chan'] != chan_data['chan']]
            
            # Transform intensities
            x_matching = transform_intensity(matching_spots[x_col], scale)
            y_matching = transform_intensity(matching_spots[y_col], scale)
            x_non_matching = transform_intensity(non_matching_spots[x_col], scale)
            y_non_matching = transform_intensity(non_matching_spots[y_col], scale)
            
            # Store data for plotting
            matching_scatter_data[chan] = (x_matching, y_matching)
            non_matching_scatter_data[chan] = (x_non_matching, y_non_matching)
        
        # Plot matching spots first (full color)
        for chan, (x, y) in matching_scatter_data.items():
            if len(x) > 0:
                ax.scatter(x, y, 
                           c=channel_colors.get(chan, 'gray'),
                           label=f'Channel {chan} (Matching)',
                           alpha=0.1,
                           s=0.5,
                           edgecolors='none')
        
        # Plot non-matching spots with darker colors
        for chan, (x, y) in non_matching_scatter_data.items():
            if len(x) > 0:
                ax.scatter(x, y, 
                           c=[darken_color(channel_colors.get(chan, 'gray'), 0.7)],
                           label=f'Channel {chan} (Non-Matching)',
                           alpha=0.2,
                           marker='x',
                           s=0.5,
                           linewidths=0.5)
    ...
    if plot_type == 'individual':
        ...
        for cell_id in cell_ids:
            ...
            for idx, (x_col, y_col, pair_title) in enumerate(channel_pairs):
                ax = axes[idx]
                
                # Plot pairwise intensities with matching/non-matching distinction
                ax = plot_pairwise_for_subset(cell_data, ax, x_col, y_col, scale)
                
                # Extract the channel numbers from column names
                x_chan = x_col.split('_')[1]
                y_chan = y_col.split('_')[1]
                
                ax.set_xlabel(f'{x_chan} Intensity ({scale} scale)')
                ax.set_ylabel(f'{y_chan} Intensity ({scale} scale)')
                ax.set_title(f'Cell {cell_id}: {pair_title}')
                ax.legend()
                
                # Add regression line
                # x_all = transform_intensity(cell_data[x_col], scale)
                # y_all = transform_intensity(cell_data[y_col], scale)
                # m, b = np.polyfit(x_all, y_all, 1)
                # ax.plot(x_all, m*x_all + b, color='purple', linestyle='--', label='Regression Line')
            
            # Add statistics
            stats_text = generate_cell_stats(cell_data)
        ...
        # Create a single plot with all cells
        fig, axes = plt.subplots(1, 3, figsize=(20, 6))
        ...
        for idx, (x_col, y_col, pair_title) in enumerate(channel_pairs):
            ...
            # Plot pairwise intensities with matching/non-matching distinction
            ax = plot_pairwise_for_subset(all_cell_data, ax, x_col, y_col, scale)
            ...
            ax.set_xlabel(f'{x_chan} Intensity ({scale} scale)')
            ax.set_ylabel(f'{y_chan} Intensity ({scale} scale)')
            ax.set_title(f'All Cells: {pair_title}')
            ax.legend()
            
        
        # Add combined statistics
        stats_text = generate_cell_stats(all_cell_data)
        plt.figtext(0.02, 0.02, stats_text, fontsize=10, 
                   bbox=dict(facecolor='white', alpha=0.8))
...
def plot_spot_round_reassignment(df, round_col='round'):
    ...
    for round_val in rounds:
        ...
        removed_spots = len(round_df[round_df['valid_spot']!=True])
    ...
    print("{:<10} {:<15} {:<20} {:<20} {:<15} {:<20} {:<20}".format(
        'Round', 'Total Spots', 'Total filtered Spots', 'Dye Line ratio Filtered', 'Dist Filtered', 'r filtered',  'Total Filtered %'))
    print("-" * 80)
    for stats in round_stats:
        reassignment_percentage = (stats['total_spots_removed_by_filters'] / stats['total_spots']) * 100
        print("{:<10} {:<15} {:<20} {:<20} {:<15} {:<20} {:<6.2f}%".format(
            stats['round'], 
            stats['total_spots'], 
...
def plot_dye_lines(ax, chan1_name, chan2_name, ratios_norm, scale='log', colors=None, all_possible_channels=None):
    ...
    if chan1_idx >= ratios_norm.shape[0] or chan2_idx >= ratios_norm.shape[0]:
        ...
        print(f"Error: Channel indices {chan1_idx}, {chan2_idx} out of range for ratios matrix shape {ratios_norm.shape}.")
        return
    ...
    x_min_raw = transform_intensity(xlim[0], scale, reverse=True)
    x_max_raw = transform_intensity(xlim[1], scale, reverse=True)
    y_min_raw = transform_intensity(ylim[0], scale, reverse=True)
    ...
    ax.plot(x_transformed, y_transformed, linestyle='-', alpha=0.7, 
...
def plot_pairwise_intensities_multi_ratios(df, cell_ids, scale='log', chan_col="unmixed_chan", title_prefix=None, same_limits=False, ratios=None):
    """
    Creates 2D scatter plots comparing channel intensities pairwise for multiple cells.
    ...
    if ratios is not None:
        ...
        if ratios.shape[0] != expected_size or ratios.shape[1] != expected_size:
            print(f"Warning: Ratios matrix dimensions {ratios.shape} do not match expected size {expected_size}x{expected_size}.")
    ...
    for i, chan1 in enumerate(available_channels):
        for j, chan2 in enumerate(available_channels):
            ...
            for unmixed_chan in plot_channels:
                ...
                if len(chan_data) > 0:
                    ...
                    ax.scatter(x, y, 
                            ...
                            s=0.1)  # Smaller points for combined view
            ...
            if same_limits:
                ...
                if len(x_all) > 0 and len(y_all) > 0:
                    if scale == 'log':
                        ...
                        if y_lim>10000: y_lim = 10000
                        if x_lim>10000: x_lim= 10000
            ...
            if ratios_norm is not None:
                ...
                plot_dye_lines(ax, chan1, chan2, ratios_norm, scale, 
                               all_possible_channels=all_possible_channels)
    ...
    for chan in sorted_channels:
        ...
        if chan_count > 0:
            ...
            marker = plt.Line2D([0], [0], marker='o', color=color, markersize=8, 
                               linestyle='', markerfacecolor=color, alpha=1.0)
    ...
    if channel_markers:
        ...
        legend = legend_ax.legend(channel_markers, channel_labels, 
                                 loc='center', title='Channels', 
    ...
    if not title_prefix and len(all_cell_data) > 0 and 'round' in all_cell_data.columns:
        ...
        fig.suptitle(f"{round_info} - Channel Intensity Comparisons", 
                    fontsize=14, y=0.95)
    elif title_prefix:
        fig.suptitle(f"{title_prefix} - Channel Intensity Comparisons", 
                    fontsize=14, y=0.95)
    else:
...
def plot_filtered_intensities(plot_df, 
                              round_n, 
                              mouse_id, 
                              plot_cell_ids=None, 
