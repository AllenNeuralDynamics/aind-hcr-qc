...
def plot_spot_metric_dist(spots_df, 
    ...
    fig, axs = plt.subplots(nrows=n_channels, ncols=2, 
                            figsize=(8, 4 * n_channels), sharex=False, sharey=False)
    ...
    for i, chan in enumerate(channels):
        ...
        # add to legends
        axs[i, 0].text(0.05, 0.95, f'Below r < {r_thresh}: {n_below_r_thresh} ({percent_below_r_thresh:.2f}%)', 
                       transform=axs[i, 0].transAxes, fontsize=12, verticalalignment='top')
        axs[i, 1].text(0.05, 0.95, f'Above dist > {dist_thresh}: {n_above_dist_thresh} ({percent_above_dist_thresh:.2f}%)',
                       transform=axs[i, 1].transAxes, fontsize=12, verticalalignment='top')
